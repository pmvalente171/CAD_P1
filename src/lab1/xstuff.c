	     ...
	     unsigned width, unsigned height)
{
  ...
  if (rc == 0) {
    ...
    /* allocate the set of colors we will want to use for the drawing. */
    rc = XAllocNamedColor(theDisplay, default_cmap, "khaki", &gold, &gold);
    if (rc == 0) {
        fprintf(stderr, "XAllocNamedColor - failed to allocated 'gold' color.\n");
        exit(1);
    }
